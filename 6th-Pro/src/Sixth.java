...
public class Sixth {
	public static void main(String[] args) {
		...
		try{
				...
				for(int i = 0; i<numberOfShips; i++){
					...
					//---------------　往路　---------------
						...
						if(ships[i].x+ships[i].v>800){
							...
							//いなければ-1にする。
							if(plant.isFree()){
								plant.setNextFT(rainyDays, ships[i].load, t+1);
								plant.isWaiting[i] = -1;
							}else{
								plant.isWaiting[i] = 1;
							}
					...
					//---------------　プラント　---------------
						...
						if(rainy){
							...
							timePlant = 0;
							//----- 戦略 3 (早めに帰る) -----
							if(ships[i].load <= 1200 && plant.Queue()){
								//雨で中断されたときに残り1200を下回る かつ 行列ができてる -> 帰る
								ships[i].v=speed.speed(ships[i], plant, t);
								plant.isWaiting[i]=0;
							}
						}else{
			...
			System.out.println("基本速度:"+sumv/numv/1.852);
